...
<template>
  <div class="contenedor">
    ...
    >
      <template v-slot:top>
        <v-toolbar flat>
          <v-toolbar-title v-if="selected.length === 0">BOM</v-toolbar-title>
          <v-divider class="mx-4" inset vertical></v-divider>
          <modal-ensamble @insertCallback="initialize()" />
      ...
      <template v-slot:[`item.actions`]="{ item }">
        <v-btn
          icon
          link
          :to="item.tipo == 'Ensamble' ? `/Ensamble/${item.id}` : null"
          v-if="item.tipo == 'Ensamble'"
        >
          <v-icon small color="info"> mdi-family-tree </v-icon>
        </v-btn>

        <v-btn
          icon
          link
          :to="item.tipo == 'Make Item' ? `/MakeItem/${item.id}` : null"
          v-if="item.tipo == 'Make Item'"
        >
...
import ModalEnsamble from "../Modals/ModalEnsamble";
export default {
  name: "BOM",
  components: { ModalEnsamble },
  data: () => ({
    dialog: false,
