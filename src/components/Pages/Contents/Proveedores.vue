  ...
  <v-container>
    ...
    >
          ...
          <v-dialog v-model="dialog" max-width="500px">
            <template v-slot:activator="{ on, attrs }">
              ...
              <modal-proveedor
                ...
                v-on:insertCallback="getProveedores"
              ></modal-proveedor>
              <v-switch
                v-model="dense"
                hide-details
      ...
      <template v-slot:[`item.proveedores`]="{ item }">
        <form
          v-for="proveedor in item.proveedores"
          :key="proveedor"
          target="_blank"
          :action="`https://www.argosonline.com.mx/api/uploads/${proveedor}`"
          method="post"
        >
          <input class="hidden" name="request" value="1" />
          <input class="hidden" name="token" :value="token" />
          <input class="hidden" name="file" :value="proveedor" />
          <button class="info--text">
            {{ proveedor }}
          </button>
        </form>
...
export default {
  ...
  data: () => ({
    ...
    headers: [
      ...
      { text: "Teléfono representante", value: "telefonoRepresentante" },
