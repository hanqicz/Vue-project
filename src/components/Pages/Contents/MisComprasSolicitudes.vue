    ...
    >
      <template v-slot:top>
        <v-toolbar flat>
          <v-toolbar-title v-if="selected.length === 0"
            >Mis solicitudes de compras</v-toolbar-title
          >
          <v-divider
            class="mx-4"
            inset
      ...
      <template v-slot:[`item.evaluaciones`]="{ item }">
        <div v-if="item.cumple == 0">
          <form
            ...
            :key="evaluacion"
            target="_blank"
            :action="`${baseURL}api/uploads/${evaluacion}`"
            method="post"
          >
      ...
      <template v-slot:[`item.status`]="{ item }">
        <v-btn
          ...
          >Orden</v-btn
        >
        <v-btn text v-else small color="error">Solicitud</v-btn>
      </template>
...
export default {
  ...
  methods: {
    ...
    async getSolicitudes() {
      this.isLoading = true;
      let myUrl = process.env.VUE_APP_API + "obtenerMisSolicitudesCompras.php";
