      ...
      <v-card :loading="isLoading">
        <div class="contenedor">
          ...
          <v-row v-if="camposVisibles">
            ...
            <v-col cols="12" sm="12" md="4">
              <v-text-field
                label="Código"
                v-model="servicioExterno.codigo"
                dense
                :rules="rule"
              ></v-text-field>
            </v-col>
            <v-col cols="12" sm="12" md="8">
              <v-text-field
                label="Descripción"
                v-model="servicioExterno.descripcion"
                dense
                :rules="rule"
              ></v-text-field>
            </v-col>
        ...
        <v-simple-table fixed-header height="76vh">
          <template>
            ...
            <tbody>
              <tr v-for="(item, ind) in serviciosExternosFiltradas" :key="ind">
...
export default {
  ...
  computed: {
    serviciosExternosFiltradas() {
      let filtradas = this.serviciosExternos.filter(hoja => {
        let isIn = false;
        let countMatches = 0;
  ...
  methods: {
    ...
    async getServicioExterno(myId) {
      ...
      localStorage.token = respuesta.data.token;
      if (respuesta.data.status == "Ok") {
        this.servicioExterno = respuesta.data.rows[0];
        this.isLoading = false;
        console.log(this.servicioExterno);
      }
    },
    async send(myId) {
      if (this.valid == false) {
    ...
    async descargarReporte() {
      ...
      this.serviciosExternosFiltradas.map((registro, i) => {
        let col = 0;
        this.headers.map(header => {
          if (i == 0) {
      ...
      XLSX.writeFile(wb, `serviciosExternos.xlsx`);
    },
    async eliminar(ind) {
      this.serviciosExternos.splice(ind, 1);
      let myUrl = process.env.VUE_APP_API + "newModules/serviciosExternos.php";
      let respuesta = await axios.post(myUrl, {
        request: 1,
        id: this.idEliminar,
