<template>
  <div class="text-left">
    ...
    >
      ...
      <v-card>
        <v-card-title>
          Agregar evaluación a {{ requisito.nombre }}</v-card-title
        >

        <v-card-text>
          <v-file-input
            label="Sube una evaluación para éste requisito"
            small-chips
            dense
...
export default {
  ...
  methods: {
    ...
    async send() {
      ...
      formData.append("idRequisito", this.requisito.id);

      let respuesta = await axios.post(
        process.env.VUE_APP_API + "uploadEvaluacion.php",
        formData,
        {
          headers: {
      ...
      if (respuesta.data.status == "Ok") {
        this.$toasted.success("Evaluación agregada").goAway(2000);
        this.$emit("closeModal");
        this.file = null;
